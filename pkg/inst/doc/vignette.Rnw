...
test_ferrari <- test.simulation(GE$data, CN$data, method="ferrari", n=100)
@
...
res <- test.geneorder.pipeline(ge = ge, cn.raw = cn.raw, cn.seg = cn.seg, cn.call = cn.call, cghCall = cgh, Labels = NULL, cancerGenes = cancerGenes, nperm=20, input = "real", version = "normal", methods = methods, references="none")
auc.ordered <- sort(unlist(res$auc))
@

<<run.simulation, results = hide>>=
methods <- c("CNAmet", "edira")
auc <- test.geneorder.pipeline(ge=test_schaefer$ge, cn.raw=test_schaefer$cn.raw, cn.seg=test_schaefer$cn,
