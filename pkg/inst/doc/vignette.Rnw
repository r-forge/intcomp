...
res.hyman <- test.geneorder.pipeline(ge = hyman$ge,           # expression
                                     cn.raw = hyman$cn.raw,   # raw copy number
				     cghCall = hyman$cghCall, # segm/called cn
				     cancerGenes = cancerGenes, 
				     methods = methods, 
				     cn.default = "raw", # use with ge 
             references="none")  # no real reference data included 
...
data(tgdb)
# Convert to EntrezIDs and select genes common with the ge/cn data
cancerGenes <- get.brca.genes(rownames(pollack$ge$data), as.list(org.Hs.egALIAS2EG), tgdb)
...
data (cn.raw/cn.seg/cn.call/cghCall), (iii) a golden standard list of
known cancer genes (cancerGenes), and (iv) the list of methods to
compare (mehods).
...
{\it PMA} \cite{Witten09}, {\it PREDA/SODEGIR}
\cite{Bicciato09,Ferrari11}.

%and %exclusively for two-group comparisons (requiring Labels) {\it
%DRI.ct}, %{\it DRI.ss}, {\it DRI.srank}, {\it DRI.sraw}
%\cite{Salari2010}. 
