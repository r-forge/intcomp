...
\section{Benchmarking the comparison methods}
...
gene detection algorithms on real experimental data, and two simulated
data sets from previous publications \cite{Bicciato09, Schafer09},
called Ferrari and Schaefer data sets, respectively. The data sets
have varying setups, depending on whether they include two-group
comparisons or segmented/called copy number data. Showcases running
the benchmarking pipeline on each data set are described below.
...
# Preprocess the Hyman data
hyman <- read.hyman(cdna, cgh, genenames, xx = as.list(org.Hs.egALIAS2EG))
GE <- hyman$ge        # gene expression data (expression and location info)
CN.raw <- hyman$cn    # copy number data (copy number and location info)
...
res.hyman <- test.geneorder.pipeline(ge = GE, cn.raw = CN.raw,
...
standard list from the TGDB (See Hyman data set). To run the
...
# Run the benchmarking pipeline
res.pollack <- test.geneorder.pipeline(ge = GE, cn.raw = CN.raw,
cn.seg = CN.raw, cn.call = CN.call, cghCall = cgh,
Labels = NULL, cancerGenes = cancerGenes, nperm=20,
input = "real", version = "normal",
methods = methods, references="none")
...
# Run the benchmarking pipeline
res.ferrari  <- test.geneorder.pipeline(ge=ferrari$ge,
cn.raw=ferrari$cn.raw, cn.seg=NULL,
cn.call=ferrari$cn.call,
cghCall = ferrari$cn.cghCall,
ge.norm = ferrari$ge.norm,
cn.norm = ferrari$cn.norm,
Labels=ferrari$Labels,
...
# Run the benchmarking pipeline
res.schaefer  <- test.geneorder.pipeline(ge=schaefer$ge,
cn.raw=schaefer$cn.raw, cn.seg=NULL,
cn.call=schaefer$cn.call,
cghCall = schaefer$cn.cghCall,
ge.norm = schaefer$ge.norm,
cn.norm = schaefer$cn.norm,
...
\section{Notes on the benchmarking pipeline}

The example data sets contains (i) gene expression data (GE), (ii)
gene copy number data (CN), (iii) optional sample class labels
(tumor/normal), and (iv) a golden standard list of known cancer genes.
...
the chromosome and chromosomal arm of the probe, respectively. 
