...
cn.raw <- pollack$cn # copy number matrix (cn$data); info (cn.raw$info)
...
Hyman and Pollack data sets, the original data sets are used (cn.seg =
cn.raw) in the experiments (except intCNGEan and CNAmet that require
segmented and called data, respectively). 
...
The cancer gene prioritization of each method is compared to the
golden standard list of known cancer genes; the result contains
